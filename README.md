...
- fixed-shape TensorRT prefill перестраивает sink-attention cache за 0.444
  секунды вместо 7.44 секунды у точного q=1 replay. На 44.16-секундной записи
  RTF снизился с 1.078 до 0.913. В слепом A/B различий в звуке не услышали.
...
- Резидентный процесс без отдельного prefill-engine занимает около 3.50 GiB
  RSS вместо 2.30 GiB у одноразового запуска. q=420 prefill-engine увеличивает
  RSS примерно до 4.36 GiB.
- Полная конфигурация с q=420 рассчитана на headless Jetson. При работающем GDM
  общего пула памяти CPU/GPU не хватает для надёжной загрузки всех engines;
  desktop display manager нужно остановить до запуска TTS. Это не влияет на
  вычисления модели и качество звука.
...
- Sink-attention rebuild после позиции 624 реализован по upstream-политике
  prompt + recent tail. Для текущего prompt длиной 108 кадров runtime одним
  вызовом q=420 TensorRT prefill восстанавливает prompt и последние 312
  temporal-входов. Если prefill-engine не задан, остаётся точный, но медленный
  q=1 replay.
- В длинных записях слышны лёгкое эхо и искажения, знакомые по более ранним
  сэмплам. Они одинаково проявились со старым replay и новым prefill, поэтому
  эту проблему пока относим к базовому генератору, голосу или Mimi, а не к
  sink-attention оптимизации.
