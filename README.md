...
[`x0rium/voxtream2-ru-jetson-onnx@v0.2.2`](https://huggingface.co/x0rium/voxtream2-ru-jetson-onnx/tree/v0.2.2).
...
- Текст на 767 входных токенов, который не помещается в TensorRT-профиль
  phone encoder, автоматически разделён по границе предложений на сегменты
  длиной 443 и 434 позиции. Runtime вернул один PCM-поток длительностью 55.76
  секунды с RTF 0.849 и без неизвестных фонем.
...
- TensorRT phone encoder намеренно собран с динамическим профилем до 640
  позиций на один сегмент. Это не ограничение общей длины синтеза: более
  длинный текст runtime автоматически делит по предложениям, знакам
  препинания или пробелам и продолжает тот же PCM-поток. На границе сегмента
  акустическое состояние сбрасывается, поэтому просодия может немного
  измениться.
